...
export function generateGalleryHTML(data) {
  ...
  const ogImageUrl = `${BASE_URL}/gallery/${slug}/og-image.jpg`;
...
function generatePhotoItem(photo, index, slug) {
  const thumbSrc = `/gallery/${slug}/${photo.thumbWebp}`;
  const fullSrc = `/gallery/${slug}/${photo.webp}`;
  const jpgSrc = `/gallery/${slug}/${photo.jpg}`;
...
function generateSchemaJSON(data) {
  ...
  if (data.photos && data.photos.length) {
    schema.subjectOf = data.photos.map((photo, i) => ({
      '@type': 'ImageObject',
      'url': `${BASE_URL}/gallery/${data.slug}/${photo.jpg}`,
      'thumbnailUrl': `${BASE_URL}/gallery/${data.slug}/${photo.thumbJpg}`,
