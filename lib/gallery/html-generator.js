...
/**
 * Generate the private gallery page (access denied — no photos).
 */
export function generatePrivateGalleryHTML(data) {
  const { slug, date, tourType, title, subtitle, bortle, objects, guests } = data;
  const url = `${BASE_URL}/gallery/${slug}/`;
  const dateFormatted = formatDate(date);
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${esc(title)} — Atacama Dark Sky</title>
  <meta name="description" content="Private tour gallery from ${dateFormatted}. Access with your personal link.">
  <link rel="canonical" href="${url}">
  <meta property="og:title" content="${esc(title)}">
  <meta property="og:description" content="Private tour gallery — access with your personal link">
  <meta property="og:url" content="${url}">
  <meta property="og:type" content="article">
  <meta property="og:site_name" content="Atacama Dark Sky">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  ...
  <link rel="icon" type="image/x-icon" href="/favicon.ico">
  <style>
    .private-container{max-width:600px;margin:0 auto;padding:8rem 1.5rem 4rem;text-align:center}
    .lock-icon{font-size:4rem;margin-bottom:1.5rem;opacity:.7}
    .private-container h1{font-size:2rem;margin-bottom:.5rem}
    .private-meta{color:var(--text-secondary);margin-bottom:2rem;font-size:1rem}
    .private-message{background:var(--bg-secondary);border:1px solid rgba(255,255,255,.06);border-radius:var(--border-radius-lg);padding:2rem;margin-bottom:2rem;line-height:1.7}
    .private-message p{margin:0 0 1rem;color:var(--text-secondary)}
    .private-message p:last-child{margin-bottom:0}
    .whatsapp-btn{display:inline-flex;align-items:center;gap:.5rem;background:#25D366;color:#fff;padding:.75rem 1.5rem;border-radius:50px;text-decoration:none;font-weight:500;transition:opacity .3s}
    .whatsapp-btn:hover{opacity:.85;color:#fff}
    .private-stats{display:flex;justify-content:center;gap:2rem;flex-wrap:wrap;margin-top:2rem;padding-top:2rem;border-top:1px solid rgba(255,255,255,.06)}
    .private-stats .stat{text-align:center}
    .private-stats .stat-number{font-size:1.4rem;font-weight:700;color:var(--primary-color)}
    .private-stats .stat-label{font-size:.75rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px}
  </style>
</head>
<body>
  ...
  <div class="private-container">
    ...
    <h1>${esc(title)}</h1>
    <p class="private-meta">${dateFormatted}${bortle ? ` &middot; Bortle Class ${bortle}` : ''}</p>

    <div class="private-message">
      <p>This gallery is private. It contains exclusive photos from this tour session.</p>
      <p>If you participated in this tour, use the link you received via WhatsApp to access your photos.</p>
      <p>Lost your link? Contact us and we'll send it again:</p>
      <a href="https://wa.me/56920441937?text=${encodeURIComponent(`Hi! I lost access to my gallery link for the tour on ${dateFormatted}. Can you send it again?`)}" class="whatsapp-btn" target="_blank" rel="noopener">
        &#128172; Contact on WhatsApp
      </a>
    </div>

    <div class="private-stats">
      ${objects && objects.length ? `<div class="stat"><div class="stat-number">${objects.length}</div><div class="stat-label">Objects Observed</div></div>` : ''}
      ${guests && guests.length ? `<div class="stat"><div class="stat-number">${guests.length}</div><div class="stat-label">Guests</div></div>` : ''}
    </div>
  </div>
