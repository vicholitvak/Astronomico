...
const VIATOR_API_KEY = process.env.VIATOR_WEBHOOK_API_KEY || '';
const VIATOR_SUPPLIER_ID = process.env.VIATOR_SUPPLIER_ID || '5073023';
...
// ============ VIATOR PUSH NOTIFICATION ============
// "Notify + Pull" model: we notify Viator that availability changed,
// they re-query our /api/viator/availability endpoint.
const VIATOR_PUSH_ENDPOINT = process.env.VIATOR_PUSH_ENDPOINT
  || 'https://toursgds.live.rc.viator.com/rest/api/supplier/json';
...
export default async function handler(req, res) {
  ...
  // Defensive unwrap: support both raw body and wrapped { requestType, data } format
  const rawBody = req.body || {};
  const data = rawBody.data && rawBody.data.ApiKey ? rawBody.data : rawBody;
...
async function handleBooking(req, res, data) {
  ...
  // Sync availability to GYG + notify Viator (non-blocking, parallel)
  syncDateAvailabilityToGYG(StartDate).catch(err => {
    console.error('[VIATOR] GYG sync failed:', err.message);
  });
  pushAvailabilityNotificationToViator(StartDate).catch(err => {
    console.error('[VIATOR] Viator push failed:', err.message);
  });
...
async function handleCancel(req, res, data) {
  ...
  pushAvailabilityNotificationToViator(dateStr).catch(err => {
    console.error('[VIATOR] Viator push failed on cancel:', err.message);
  });

  // Refresh Google Calendar (non-blocking) - remove or recalculate event
  import('../lib/google-calendar.js').then(({ refreshCalendarAfterCancellation }) => {
    refreshCalendarAfterCancellation(dateStr, tourType).catch(err => {
      console.error('[VIATOR] Calendar refresh failed on cancel:', err.message);
    });
  }).catch(err => {
    console.error('[VIATOR] Failed to import calendar module:', err.message);
  });
...
async function handleAmend(req, res, data) {
  ...
  pushAvailabilityNotificationToViator(oldDateStr).catch(err => {
    console.error('[VIATOR] Viator push failed on amend (old date):', err.message);
  });
  ...
  if (NewStartDate && NewStartDate !== oldDateStr) {
    ...
    pushAvailabilityNotificationToViator(NewStartDate).catch(err => {
      console.error('[VIATOR] Viator push failed on amend (new date):', err.message);
    });
  }
...
// ============ PUSH AVAILABILITY NOTIFICATION TO VIATOR ============
/**
 * Notify Viator that availability has changed for a given date.
 * Uses the "Notify + Pull" model: sends a lightweight POST to Viator's
 * Event Notification endpoint, then Viator re-queries our /availability endpoint.
 *
 * Fire-and-forget — errors are logged but never thrown.
 */
async function pushAvailabilityNotificationToViator(date) {
  const apiKey = process.env.VIATOR_WEBHOOK_API_KEY;
  if (!apiKey) {
    // Supplier API not configured yet — skip silently
    return;
  }
  ...
  try {
    const payload = {
      ...
      productOptionIds: ['5624520P1'],
      startDate: date,
      endDate: date
    };

    console.log(`[VIATOR-PUSH] Notifying availability change for ${date}`);

    const response = await fetch(VIATOR_PUSH_ENDPOINT, {
      method: 'POST',
      headers: {
    ...
    console.log(`[VIATOR-PUSH] Response ${response.status}: ${text}`);
  } catch (error) {
    console.error(`[VIATOR-PUSH] Failed for ${date}:`, error.message);
  }
}
...
export { PRODUCTS as VIATOR_PRODUCTS, pushAvailabilityNotificationToViator };
