...
// ============ CONFIGURATION ============
const PRODUCTS = {
  // Tour Regular (grupo, precio por persona) — Push Availability NO activo en GYG
  // NOTA: El ID 1152147 es inválido en la API de GYG (devuelve INVALID_PRODUCT).
  // GYG sí llama a nuestro endpoint de availability con este ID.
  // Esta config permite responder correctamente cuando GYG consulta disponibilidad.
  '1152147': {
    productId: '1152147',
    name: 'Atacama: Stargazing Tour',
    maxCapacity: 8,
    tourType: 'regular',
    currency: 'EUR',
    pricePerPerson: 7000, // aprox €70
    cutoffSeconds: 14400,
    availableTimes: ['21:00'],
    openingHours: { fromTime: '20:30', toTime: '23:59' },
    timePeriod: false,
...
async function handleReserve(req, res) {
  ...
  // For timePeriod products, GYG sends T00:00:00 (opening-hours format), default to 21:00
  const timeMatch = dateTime.match(/T(\d{2}:\d{2})/);
  let time = timeMatch ? timeMatch[1] : '21:00';
